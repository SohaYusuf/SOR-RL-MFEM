...
from functions.plot_policy import plot_policy_heatmap, plot_policy_with_trajectories
from functions.read_data import get_train_loader
from functions.run_baselines_sor import plot_baseline_residuals_sor, run_baselines_sor
from functions.utils import device, plot_durations, plot_omega_over_episodes, plot_results, plot_rewards


def train_sor(config):
    ...
    args = {k: config[k] for k in ["seed", "batch_size", "gamma", "eps_start", "eps_end", "eps_decay",
                               ...
                               "n_actions", "train_RL_model", "run_FGMRES_baseline", 
    ...
    train_loader = get_train_loader(train_path=config["data_path"], 
    ...
    fname = os.path.join(save_path, "all_residuals_dict_baselines_sor.npz")
    ...
    if os.path.exists(fname):
        ...
        all_residuals_dict = run_baselines_sor(config, train_loader)
    ...
    plot_baseline_residuals_sor(all_residuals_dict, save_path)
    ...
    if config["train_RL_model"]==True:
        ...
        for A, A_tensor, x_true, b in train_loader:
            ...
            omega_opt_symmetric = all_residuals_dict["optimal_omegas"][N]
            ...
            for i_episode in range(num_episodes):
                ...
                for t in count():
                    ...
                    observation, omega, reward, done, residual_list, time_list = env.step(action.item(), A, b)
                    ...
                    if done:
                        ...
                        plot_rewards(episode_rewards, train_results_folder, is_ipython)
                        break

                # omegas_over_episodes.append(float(np.mean(omega_list)))
                training_states_list.append(state)
                omegas_over_episodes.append(omega)
                actions_list.append(action.item())
                ...
                # if i_episode % 100 == 0:
                    ...
                    # # pdb.set_trace()
                    # plot_results(all_residuals_dict, 
                    #              N, 
                    #              rl_omega=omega, 
            ...
            plot_omega_over_episodes(omegas_over_episodes, 'episdoe', 'omega', 'omega', 
                              f'{save_path}/omega_over_episodes.png', config["default_omega"])
            
            plot_omega_over_episodes(omegas_over_episodes, 'episdoe', 'omega', 'omega', 
                              f'{save_path}/omega_over_episodes.png', config["default_omega"], log=True)
        ...
        # 1) decision map over default grid
        fig, ax = plot_policy_heatmap(policy_net, omega_values=np.linspace(0.0, 2.0, n_actions + 2)[1:-1],
                                    rho_bounds=(0,1.0), iter_bounds=(1,N),
                                    grid_size=(300,300), device=device,
                                    savepath=f'{save_path}/policy_map.png')
        pdb.set_trace()
            
    return policy_net, train_loader, all_residuals_dict