...
# python ex16_RL.py -m star.mesh -r 2 -dt 0.01
'''
for w in 2.5 3.0 
do
    echo "Running with omega = $w"

    sed -i "s/PyJacobiPreconditioner(omega=[0-9.]\+)/PyJacobiPreconditioner(omega=$w)/" ex16_RL.py

    python ex16_RL.py -s 22 -r 2 -m star.mesh
...
from mfem_utils import PyJacobiPreconditioner
...
class ConductionOperator(mfem.PyTimeDependentOperator):
    def __init__(self, fespace, alpha, kappa, u):
        ...
        self.T_prec = PyJacobiPreconditioner(omega=3.0)
    ...
    #         self.T = mfem.Add(1.0, self.Mmat, dt, self.Kmat)
    #         current_dt = dt
    #         # make sure the preconditioner knows the new matrix too
    #         try:
    #             self.T_prec.SetOperator(self.T)
    #         except Exception:
    #             # fallback: some preconditioners auto-discover the operator on solver.SetOperator
    #             pass
    #         self.T_solver.SetOperator(self.T)
...
##################################################################
# --- ParaView output (add this) ---
if args.paraview_datafiles:
    pd = mfem.ParaViewDataCollection("Example16", mesh)
    pd.SetPrefixPath("ParaView")               # output folder: ./ParaView
    pd.RegisterField("solution", u_gf)         # pass the GridFunction, not true DOFs
    pd.SetLevelsOfDetail(order)                # use FE order for high-order outputs
    pd.SetDataFormat(mfem.VTKFormat_BINARY)    # or VTKFormat_ASCII
    pd.SetHighOrderOutput(True)                # write high-order elements (if mesh has them)
    pd.SetCycle(0)
    pd.SetTime(0.0)
    pd.Save()
# -------------------------
